  ...
  const pieOption = useMemo(() => {
    ...
    return {
      ...
      series: [{
        ...
        radius: ['42%', '70%'],
        center: ['50%', '50%'],
        label: { show: true, fontSize: 10, color: '#555', formatter: (p: { name: string; percent: number }) => `${p.name} ${p.percent.toFixed(0)}%`, overflow: 'truncate', width: 60 },
        labelLine: { show: true, length: 10, length2: 6, lineStyle: { color: '#bbb' } },
